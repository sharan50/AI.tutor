...
@@horizon_months|int@@ months are negative — though the count alone would not settle
...
and that is the single largest reason only @@breakeven_rows_bracketed_on_cash|int@@ of the
@@breakeven_rows_against_a_cash_target|int@@ break-evens solved against a cash target brackets at all.
...
households. @@por_share_paths_over_rota_extended|pct1@@ per cent of paths sit above
the first threshold **at the final month** and @@por_share_paths_over_rota_24_7|pct1@@ per cent above the
...
@@breakeven_rows_bracketed_on_cash|int@@ of the @@breakeven_rows_against_a_cash_target|int@@ break-evens solved in section 10 against a
...
@@scenario_por_no_schools_final_year_allin_contrib_median|num2@@.

**Those two median figures are on a different path set from the
@@final_year_contrib_per_hh_month_allin_median|num2@@ in the table above, and the
...
at @@por_mean_trough_month|num1@@, and called the difference "later" by four
tenths of a month. Both halves of that are now wrong: the averaged line troughs
@@por_averaged_line_trough_gap_months|num1@@ months **earlier** than the average
...
@@twoway_content_step_profitability_at_low_cac|pct1@@ points at the cheapest
acquisition level and @@twoway_content_step_profitability_at_high_cac|pct1@@
...
**@@breakeven_gtm_minimum_uk_one_board_price_uk_tut_gbp_terminal_cash_median|num2@@ pounds a month**.
...
@@breakeven_gtm_minimum_uk_one_board_price_uk_tut_gbp_terminal_cash_median|num2@@.
...
@@breakeven_rows_bracketed_on_profitability|int@@ of the @@breakeven_rows_bracketed|int@@ are on one target, half of all paths
running three consecutive cash-positive months; the @@breakeven_rows_bracketed_on_cash|int@@ remaining one is the
...
target the @@breakeven_rows_bracketed_on_profitability|int@@ profitability solves aim at is that half of all paths run three consecutive
...
@@delta_gtm_minimum_terminal_cash_mean|usd0@@ against that, which differs from it
...
@@delta_por_content_frozen_terminal_cash_mean|usd0@@ at the mean and
@@scenario_por_content_frozen_pathwise_p50_delta|usd0@@ on the median path: it
helps on both. Dropping the second and third markets is worth
@@delta_ukonly_terminal_cash_mean|usd0@@ at the mean — it **costs** money — and
@@scenario_ukonly_pathwise_p50_delta|usd0@@ on the median path. Neither of the
market-drop figures is noise: the mean sits
@@ukonly_mean_delta_sigma|num1@@ paired standard errors below zero and the median
sits @@ukonly_median_delta_sigma|num0@@ above it.
...
| `out/invariants.csv` | The structural checks on the month loop, run on @@invariant_run_count|int@@ configurations — the plan of record and the enforced-allowance scenario, not on every scenario — with what each one measured. |
